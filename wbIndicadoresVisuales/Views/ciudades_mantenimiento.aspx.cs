...
{
    public partial class ciudades_mantenimiento : System.Web.UI.Page
    {
        ...
        public static string buscar(string QUERY, int INDEX, int CANTIDAD, int? ESTADO)
        {
            ...
            try
            {
                ...
                dbProcedimientos.CIUDADES_BUSCARDataTable dataTable = DOMModel.CIUDADES_BUSCAR(QUERY, INDEX, CANTIDAD, ref cantidad_registros, ESTADO);
                if (dataTable != null)
                {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = false)]
        public static string insertar(string txtCiudad, int cbPais)
        {
            clsResult objResultado = new clsResult();
            try
            {
                bool op = DOMModel.CIUDADES_INSERTAR(txtCiudad, cbPais);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = false)]
        public static string modificar(int ID, string txtCiudad, int cbPais)
        {
            clsResult objResultado = new clsResult();
            try
            {
                bool op = DOMModel.CIUDADES_MODIFICAR(ID, txtCiudad, cbPais);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        public static string modificar_estado(int ID, int ESTADO)
        {
            ...
            try
            {
                bool op = DOMModel.CIUDADES_MODIFICAR_ESTADO(ID, ESTADO);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        public static string eliminar(int ID)
        {
            ...
            try
            {
                bool op = DOMModel.CIUDADES_ELIMINAR(ID);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
