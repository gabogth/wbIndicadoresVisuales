...
{
    public partial class categoria_mantenimiento : System.Web.UI.Page
    {
        ...
        public static string buscar(string QUERY, int INDEX, int CANTIDAD, int? ESTADO)
        {
            ...
            try
            {
                ...
                dbProcedimientos.CATEGORIA_BUSCARDataTable dataTable = DOMModel.CATEGORIA_BUSCAR(QUERY, INDEX, CANTIDAD, ref cantidad_registros, ESTADO);
                if (dataTable != null)
                {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = false)]
        public static string insertar(string txtIcon, string txtCategoria)
        {
            clsResult objResultado = new clsResult();
            try
            {
                bool op = DOMModel.CATEGORIA_INSERTAR(txtIcon, txtCategoria);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = false)]
        public static string modificar(int ID, string txtIcon, string txtCategoria)
        {
            clsResult objResultado = new clsResult();
            try
            {
                bool op = DOMModel.CATEGORIA_MODIFICAR(ID, txtIcon, txtCategoria);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        public static string modificar_estado(int ID, int ESTADO)
        {
            ...
            try
            {
                bool op = DOMModel.CATEGORIA_MODIFICAR_ESTADO(ID, ESTADO);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        public static string eliminar(int ID)
        {
            ...
            try
            {
                bool op = DOMModel.CATEGORIA_ELIMINAR(ID);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
