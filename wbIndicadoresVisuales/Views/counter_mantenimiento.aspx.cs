...
{
    public partial class counter_mantenimiento : System.Web.UI.Page
    {
        ...
        public static string buscar(string QUERY, int INDEX, int CANTIDAD, int? ESTADO)
        {
            ...
            try
            {
                ...
                dbProcedimientos.COUNTER_BUSCARDataTable dataTable = DOMModel.COUNTER_BUSCAR(QUERY, INDEX, CANTIDAD, ref cantidad_registros, ESTADO);
                if (dataTable != null)
                {
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = false)]
        public static string insertar(string txtCounter, string txtDescripcion, int? cbEquipo)
        {
            clsResult objResultado = new clsResult();
            try
            {
                bool op = DOMModel.COUNTER_INSERTAR(txtCounter, txtDescripcion, cbEquipo == -1 ? null : cbEquipo);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = false)]
        public static string modificar(int ID, string txtCounter, string txtDescripcion, int? cbEquipo)
        {
            clsResult objResultado = new clsResult();
            try
            {
                bool op = DOMModel.COUNTER_MODIFICAR(ID, txtCounter, txtDescripcion, cbEquipo == -1 ? null : cbEquipo);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        public static string modificar_estado(int ID, int ESTADO)
        {
            ...
            try
            {
                bool op = DOMModel.COUNTER_MODIFICAR_ESTADO(ID, ESTADO);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
        ...
        public static string eliminar(int ID)
        {
            ...
            try
            {
                bool op = DOMModel.COUNTER_ELIMINAR(ID);
                if (op != false)
                {
                    objResultado.result = "success";
                    objResultado.message = "ok_server";
