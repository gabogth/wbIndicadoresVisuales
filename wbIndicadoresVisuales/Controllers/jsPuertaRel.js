...
function appendHTML(items, index) {
    ...
    $.each(items, function (ind, item) {
        ...
        html += '<td>' + item.puerta + '</td>';
        html += '<td title="' + item.decripcion + '">' + (item.decripcion.length > 20 ? item.decripcion.substring(0, 19) : item.decripcion) + '</td>';
        html += '<td>' + (item.nombre == null ? 'Ninguno' : item.nombre) + '</td>';
        html += '<td>' + (item.ip == null ? 'Ninguno' : item.ip) + '</td>';
        html += '<td class="text-center" id="estado-' + item.idpuerta + '" style="cursor: pointer;">' + getEstado(item.estado) + '</td>';
        html += '<td class="text-center" id="modificar-' + item.idpuerta + '" style="cursor: pointer;"><span class="fa fa-pencil"></span></td>';
        html += '<td class="text-center" id="eliminar-' + item.idpuerta + '" style="cursor: pointer;"><span class="fa fa-trash"></span></td>';
        html += '<tr>';
    });
...
function appendMethods(items) {
    $.each(items, function (ind, item) {
        $('#modificar-' + item.idpuerta).unbind();
        $('#modificar-' + item.idpuerta).on('click', function (e) {
            $('#txtPuerta').val(item.puerta);
            $('#txtDescripcion').val(item.decripcion);
            setSelect2('#cbEquipo', item.idequipo, '[' + item.ip + ']-' + item.nombre);
            $('#modalInsertar').modal('show');
            $('#myModalLabel').html('Modificar ' + item.puerta + ' - ' + item.ip);
            $('#frmInsertar').unbind();
            $('#frmInsertar').submit(function (event) {
                modificar(item.idpuerta);
                event.preventDefault();
            });
        });
        $('#eliminar-' + item.idpuerta).unbind();
        $('#eliminar-' + item.idpuerta).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            $('#bodyMessage').html('¿Está seguro que desea eliminar a <b>' + item.puerta + ' - ' + item.ip + '</b>?');
            $('#modalQuest').modal('show');
            $('#btnOK').unbind();
            $('#btnOK').on('click', function (e) {
                eliminar(item.idpuerta);
            });
        });
        $('#estado-' + item.idpuerta).unbind();
        $('#estado-' + item.idpuerta).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            $('#bodyMessage').html('¿Está seguro que desea cambiar el estado a <b>' + item.puerta + ' - ' + item.ip + '</b>?');
            $('#modalQuest').modal('show');
            $('#btnOK').unbind();
            $('#btnOK').on('click', function (e) {
                var estado = 1 - item.estado;
                modificar_estado(item.idpuerta, estado);
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'puerta_relacionar.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(ID) {
    ...
    $.ajax({
        url: 'puerta_relacionar.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData)
...
function modificar_estado(ID, ESTADO) {
    $.ajax({
        url: 'puerta_relacionar.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(ID) {
    $.ajax({
        url: 'puerta_relacionar.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + ID + '}'
