...
function buscar(index) {
    $.ajax({
        url: 'fids_relacionar.aspx/buscar',
        type: "POST",
        dataType: 'json',
...
function appendHTML(items, index) {
    ...
    $.each(items, function (ind, item) {
        ...
        html += '<td>' + (item.salida_llegada == 0 ? 'SALIDA' : 'LLEGADA') + '</td>';
        html += '<td class="text-center" id="estado-' + item.idfid + '" style="cursor: pointer;">' + getEstado(item.estado) + '</td>';
        html += '<td class="text-center" id="modificar-' + item.idfid + '" style="cursor: pointer;"><span class="fa fa-pencil"></span></td>';
        html += '<td class="text-center" id="eliminar-' + item.idfid + '" style="cursor: pointer;"><span class="fa fa-trash"></span></td>';
        html += '<tr>';
    });
...
function appendMethods(items) {
    $.each(items, function (ind, item) {
        $('#modificar-' + item.idfid).unbind();
        $('#modificar-' + item.idfid).on('click', function (e) {
            $('#cbIO').val(item.salida_llegada);
            setSelect2('#cbEquipo', item.idequipo, '[' + item.ip + ']-' + item.nombre);
            $('#modalInsertar').modal('show');
            $('#myModalLabel').html('Modificar ' + item.nombre + ' - ' + item.ip);
            $('#frmInsertar').unbind();
            $('#frmInsertar').submit(function (event) {
                modificar(item.idfid);
                event.preventDefault();
            });
        });
        $('#eliminar-' + item.idfid).unbind();
        $('#eliminar-' + item.idfid).on('click', function (e) {
        ...
        $('#estado-' + item.idfid).unbind();
        $('#estado-' + item.idfid).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            ...
            $('#btnOK').on('click', function (e) {
                ...
                modificar_estado(item.idfid, estado);
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'fids_relacionar.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(ID) {
    ...
    $.ajax({
        url: 'fids_relacionar.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData)
...
function modificar_estado(ID, ESTADO) {
    $.ajax({
        url: 'fids_relacionar.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(ID) {
    $.ajax({
        url: 'fids_relacionar.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + ID + '}'
