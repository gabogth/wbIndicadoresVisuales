...
$(function () {
    ...
    function appendHTML(items, objFID) {
        ...
        animate(htmlLlegadas, 'ARRIVALS', 'LLEGADAS');
    }

    function animate(htmlSalidas, headerEn, headerEs) {
        clearInterval(timerID);
        $('#fid_anuncios div').fadeOut(500).promise().done(function () {
            $('#fid_anuncios').html(htmlSalidas);
            $('#fid_anuncios div').fadeOut(0);
            $('#fid_anuncios div').fadeIn(500).promise().done(function () {
                $('#fid_anuncios').html(htmlSalidas);
            });
        });
        $('#lblTipo').attr('data-es', headerEs);
        $('#lblTipo').attr('data-en', headerEn);
        $('#lblTipo').html('<span>' + headerEs + '</span>');
        timerID = setInterval(function () {
            if (lang_toggle) {
                $('#fid_anuncios div, #lblTipo').each(function (indice, item) {
    ...
    function getHTML(data, type) {
        ...
        html = '<div class="row cabecera">\n' +
                    ...
                    '<div class="col-lg-3 col-md-3 col-sm-3 nestun text-center" ' + (type == 0 ? 'data-en="Destination" data-es="Destino"' : 'data-en="From" data-es="Origen"') + '>\n' +
                        (type == 0 ? '<span style="text-shadow: -2px 0 black, 0 2px black, 2px 0 black, 0 -2px black;">Destino</span>' : '<span style="text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black;">Origen</span>') +
                    '\n</div>' +
                    '<div class="col-lg-1 col-md-1 col-sm-1 nestun text-center" data-en="Belt" data-es="Faja">\n' +
                        '<span style="text-shadow: -2px 0 black, 0 2px black, 2px 0 black, 0 -2px black;">Faja</span>\n' +
                    '</div>\n' +
                ...
                if (item.AVFID_enable == 1) {
                    if (counter < 8) {
                        html += '<div class="row ' + (isFirst ? 'body_first' : 'body_large') + '">\n' +
                                ...
                                '</div>\n' +
                                '<div class="col-lg-1 col-md-1 col-sm-1 nestun text-center" style="text-shadow: -2px 0 black, 0 2px black, 2px 0 black, 0 -2px black;">\n' +
                                    (item.AVFID_FAJ_faja != null ? item.AVFID_FAJ_faja : '') + '\n' +
                                '</div>\n' +
