...
function appendHTML(items, index) {
    ...
    $.each(items, function (ind, item) {
        ...
        html += '<td class="text-center" id="estado-' + item.idpais + '" style="cursor: pointer;">' + getEstado(item.estado) + '</td>';
        html += '<td class="text-center" id="modificar-' + item.idpais + '" style="cursor: pointer;"><span class="fa fa-pencil"></span></td>';
        html += '<td class="text-center" id="eliminar-' + item.idpais + '" style="cursor: pointer;"><span class="fa fa-trash"></span></td>';
        html += '<tr>';
    });
...
function appendMethods(items) {
    $.each(items, function (ind, item) {
        $('#modificar-' + item.idpais).unbind();
        $('#modificar-' + item.idpais).on('click', function (e) {
            $('#txtPais').val(item.pais);
            $('#txtAbrev').val(item.abrev);
            $('#modalInsertar').modal('show');
            $('#myModalLabel').html('Modificar ' + item.pais);
            $('#frmInsertar').unbind();
            $('#frmInsertar').submit(function (event) {
                modificar(item.idpais);
                event.preventDefault();
            });
        });
        $('#eliminar-' + item.idpais).unbind();
        $('#eliminar-' + item.idpais).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            $('#bodyMessage').html('¿Está seguro que desea eliminar a <b>' + item.pais + '</b>?');
            $('#modalQuest').modal('show');
            $('#btnOK').unbind();
            $('#btnOK').on('click', function (e) {
                eliminar(item.idpais);
            });
        });
        $('#estado-' + item.idpais).unbind();
        $('#estado-' + item.idpais).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            $('#bodyMessage').html('¿Está seguro que desea cambiar el estado a <b>' + item.pais + '</b>?');
            $('#modalQuest').modal('show');
            $('#btnOK').unbind();
            $('#btnOK').on('click', function (e) {
                var estado = 1 - item.estado;
                modificar_estado(item.idpais, estado);
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'pais_mantenimiento.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(ID) {
    ...
    formData["ID"] = ID;
    $.ajax({
        url: 'pais_mantenimiento.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData)
...
function modificar_estado(ID, ESTADO) {
    $.ajax({
        url: 'pais_mantenimiento.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(ID) {
    $.ajax({
        url: 'pais_mantenimiento.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + ID + '}'
