...
function appendHTML(items, index) {
    ...
    $.each(items, function (ind, item) {
        ...
        html += '<td class="text-center" id="estado-' + item.idfaja + '" style="cursor: pointer;">' + getEstado(item.estado) + '</td>';
        html += '<td class="text-center" id="modificar-' + item.idfaja + '" style="cursor: pointer;"><span class="fa fa-pencil"></span></td>';
        html += '<td class="text-center" id="eliminar-' + item.idfaja + '" style="cursor: pointer;"><span class="fa fa-trash"></span></td>';
        html += '<tr>';
    });
...
function appendMethods(items) {
    $.each(items, function (ind, item) {
        $('#modificar-' + item.idfaja).unbind();
        $('#modificar-' + item.idfaja).on('click', function (e) {
            $('#txtFaja').val(item.faja);
            $('#txtDescripcion').val(item.descripcion);
            setSelect2('#cbEquipo', item.idequipo, '[' + item.ip + ']-' + item.nombre);
            $('#modalInsertar').modal('show');
            $('#myModalLabel').html('Modificar ' + item.faja + ' - ' + item.ip);
            $('#frmInsertar').unbind();
            $('#frmInsertar').submit(function (event) {
                modificar(item.idfaja);
                event.preventDefault();
            });
        });
        $('#eliminar-' + item.idfaja).unbind();
        $('#eliminar-' + item.idfaja).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            $('#bodyMessage').html('¿Está seguro que desea eliminar a <b>' + item.faja + ' - ' + item.ip + '</b>?');
            $('#modalQuest').modal('show');
            $('#btnOK').unbind();
            $('#btnOK').on('click', function (e) {
                eliminar(item.idfaja);
            });
        });
        $('#estado-' + item.idfaja).unbind();
        $('#estado-' + item.idfaja).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            $('#bodyMessage').html('¿Está seguro que desea cambiar el estado a <b>' + item.faja + ' - ' + item.ip + '</b>?');
            $('#modalQuest').modal('show');
            $('#btnOK').unbind();
            $('#btnOK').on('click', function (e) {
                var estado = 1 - item.estado;
                modificar_estado(item.idfaja, estado);
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'faja_mantenimiento.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(ID) {
    ...
    $.ajax({
        url: 'faja_mantenimiento.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData)
...
function modificar_estado(ID, ESTADO) {
    $.ajax({
        url: 'faja_mantenimiento.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(ID) {
    $.ajax({
        url: 'faja_mantenimiento.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + ID + '}'
