$(function () {
    ...
    $('#btnAdd').on('click', function (e) {
        $('#myModalLabel').html('Nuevo Pais');
        $('#txtCiudad').val('');
        $('#cbPais').select2('val', null);
...
function appendHTML(items, index) {
    ...
    $.each(items, function (ind, item) {
        ...
        html += '<td>[' + item.abrev_pais + ']-' + item.pais + '</td>';
        html += '<td class="text-center" id="estado-' + item.idciudad + '" style="cursor: pointer;">' + getEstado(item.estado) + '</td>';
        html += '<td class="text-center" id="modificar-' + item.idciudad + '" style="cursor: pointer;"><span class="fa fa-pencil"></span></td>';
        html += '<td class="text-center" id="eliminar-' + item.idciudad + '" style="cursor: pointer;"><span class="fa fa-trash"></span></td>';
        html += '<tr>';
    });
...
function appendMethods(items) {
    $.each(items, function (ind, item) {
        $('#modificar-' + item.idciudad).unbind();
        $('#modificar-' + item.idciudad).on('click', function (e) {
            $('#txtCiudad').val(item.ciudad);
            setSelect2('#cbPais', item.idpais, item.pais);
            $('#modalInsertar').modal('show');
            $('#myModalLabel').html('Modificar ' + item.ciudad);
            $('#frmInsertar').unbind();
            $('#frmInsertar').submit(function (event) {
                modificar(item.idciudad);
                event.preventDefault();
            });
        });
        $('#eliminar-' + item.idciudad).unbind();
        $('#eliminar-' + item.idciudad).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            $('#bodyMessage').html('¿Está seguro que desea eliminar a <b>' + item.ciudad + '</b>?');
            $('#modalQuest').modal('show');
            $('#btnOK').unbind();
            $('#btnOK').on('click', function (e) {
                eliminar(item.idciudad);
            });
        });
        $('#estado-' + item.idciudad).unbind();
        $('#estado-' + item.idciudad).on('click', function (e) {
            $('#questTitulo').html('Advertencia');
            $('#bodyMessage').html('¿Está seguro que desea cambiar el estado a <b>' + item.ciudad + '</b>?');
            $('#modalQuest').modal('show');
            $('#btnOK').unbind();
            $('#btnOK').on('click', function (e) {
                var estado = 1 - item.estado;
                modificar_estado(item.idciudad, estado);
            });
        });
    });
}
...
function insertar() {
    ...
    $.ajax({
        url: 'ciudades_mantenimiento.aspx/insertar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function modificar(ID) {
    ...
    formData["ID"] = ID;
    $.ajax({
        url: 'ciudades_mantenimiento.aspx/modificar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(formData)
...
function modificar_estado(ID, ESTADO) {
    $.ajax({
        url: 'ciudades_mantenimiento.aspx/modificar_estado',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
...
function eliminar(ID) {
    $.ajax({
        url: 'ciudades_mantenimiento.aspx/eliminar',
        type: "POST",
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        data: '{"ID": ' + ID + '}'
...
function cargarPais() {
    $('#cbPais').select2({
        ajax: {
            url: 'pais_mantenimiento.aspx/buscar',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            type: 'POST',
            ...
            processResults: function (data, page) {
                ...
                if (datos != null && datos.length > 0) {
                    for (var i = 0; i < datos.length; i++) {
                        datos[i].id = datos[i].idpais;
                        datos[i].text = datos[i].pais;
                    }
                }
                return {
                    results: datos
